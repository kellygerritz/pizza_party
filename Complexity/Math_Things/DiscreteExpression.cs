...
namespace Complexity.Math_Things {
    public class DiscreteExpr {
        ...
        public static void AddSymbol(string name, double value) {
            SYMBOLS.Add(name, new Symbol(name, false, 0, LEFT_ASSOC, 0, (a) => value));
        }
        ...
        public DiscreteExpr(string infix) {
        ...
        public string[] Parse(string input) {
            ...
            input = input.Replace("\t", "");
            input = input.Replace("\n", "");
            string[] tokens = Regex.Split(input, @"([a-z]+|[\\(]|[\\)]|" + OPS_REGEX + ")");

            //Convert and test
            ArrayList _tokens = new ArrayList();
            int parens = 0;
            foreach (string token in tokens) {
                if (IsDefined(token) || IsNumeric(token)) {
                    _tokens.Add(token);
